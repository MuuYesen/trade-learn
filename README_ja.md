...
**trade-learn** は、**投資研究 (Learn)** と **取引バックテスト (Trade)** を高度に統合したクオンツ投資研究フレームワークです。インデックス強化、機械学習戦略、および複雑なイベント駆動型シナリオ向けに設計されており、「Python によるロジック表現 + Rust によるコア駆動」のハイブリッドアーキテクチャを採用しています。Python による投資研究の柔軟性を維持しつつ、Rust ネイティブコアによってバックテストのパフォーマンスを極限まで高め、ファクターマイニングから因果探索、実験追跡に至る全プロセスが、再現可能な工業レベルのワークフローの中で完結することを保証します。

それは、単に「バックテストを実行する方法」という従来のバックテストフレームワークの限界を打ち破り、断片化されたファクターマイニング、データアライメント、戦略バックテスト、および監査追跡を、完全なライフサイクル投資研究パイプラインに統合します。
...
**trade-learn** は単なる機能の積み重ねではなく、独自の「双模双核（デュアルモード・デュアルコア）」設計を通じて、専門的な深みと研究効率のバランスを構築します。下層の **Engine** は Backtrader のセマンティクスと高度に同期しており、論理的な正確さを保証します。一方、上層の **Lite** は Pythonic でミニマリストなインターフェースを提供します。

開発段階に応じて、戦略の表現の深さを柔軟に選択できます：
- **Engine モード (深度同期)**: Analyzer/Sizer/Signal エコシステムを完全にサポートし、論理的に精密で非常に細かい粒度を持つ本番級のシステムの構築に特化しています。
- **Lite モード (アジャイル反復)**: 極限のミニマリズムを追求し、モデルの重み（Weights）との直接連携をサポートします。ファクターマイニング段階での高頻度な反復やプロトタイプ検証のために作成されました。

さらに、このプロジェクトは **因果推論 (Causal Inference)** をファクター研究に革新的に導入しています。内蔵の `CausalSelector` と連携することで、特徴量選定、パラメータ最適化、およびバックテスト監査を自動的に接続し、透明で科学的、かつ高度に自動化された投資研究パイプラインを提供します。
...
- **Lite のミニマリズム表現**: 同一の Runtime 上に構築された軽量な構文。**内蔵 of `target_weights` インターフェース**により、機械学習モデルが出力した重みをワンクリックでバックテストの意思決定に変換します。
...
*   **🌐 跨市場・多戦略チーム**
    A株 (TDX) と海外 (TradingView) の両方をカバーし、インジケーター基準とレポート体系の完全な一致を求めるチーム。
*   **🔍 因果推論の探求者**
