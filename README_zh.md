...
你可以像写 Backtrader 一样写专业策略，也可以用 Lite API 快速验证一个想法；可以接入 TDX、TA-Lib、TradingView、pandas-ta-classic 等指标生态，也可以把因子分析、组合权重、回测报告和实验记录放进同一条工作流。
...
- **Lite 快速策略层**：更短的写法，适合快速验证、教学和 1.x 风格迁移。
- **Rust 回测内核**：撮合、bar loop、订单推进和 portfolio 状态走 Rust，Python 策略 API 保持清晰。
- **指标生态统一**：内置 `talib`、`tdx`、`tv`、`pta` 命名空间，指标计算仍然留在 Python 生态，便于扩展和核对口径。
- **研究流水线**：`FeatureSet`、`Pipeline`、`ResearchRun`、`WeightBuilder` 把 train/test、preprocess、score、weights 和回测串起来。
...
`examples/engine` 下的 Backtrader 风格示例会进入 `benchmark_bt.py` 做 oracle 审计。examples 的 K 线数量较小，时间对比不稳定，因此这里只展示对齐指标和最终结论。

| Engine 示例策略 | 对齐指标 | 状态 |
| --- | --- | --- |
| QuickstartSmaCross | Final Value / Closed Trades / PnL | EXACT |
| SmaCross | Final Value / Closed Trades / PnL | EXACT |
| MigratedSmaCross | Final Value / Closed Trades / PnL | EXACT |
| Turtle | Final Value / Closed Trades / PnL | EXACT |
| EnhancedRSI | Final Value / Closed Trades / PnL | EXACT |
| BetterMA | Final Value / Closed Trades / PnL | EXACT |
| MacdTharp | Final Value / Closed Trades / PnL | EXACT |
| OrderExecutionStrategy | Final Value / Closed Trades / PnL | EXACT |

`examples/lite` 不作为 Backtrader facade 逐策略对齐，但必须证明能正确接入同一 runtime。当前 smoke 覆盖 Lite 示例和组合示例，最近一次结果为 `5 passed`；Lite 的时间对比和统计口径主要看上方大样本性能表。
...
| 多标的策略 | 对齐指标 | 状态 |
| --- | --- | --- |
| TargetPercentPortfolioStrategy | Final Value / Orders | EXACT |
| AssetClassTargetPortfolioStrategy | Final Value / Orders | EXACT |
| UniformAssetClassPortfolioStrategy | Final Value / Orders | EXACT |
| TrendFilteredPortfolioStrategy | Final Value / Orders | EXACT |
| InverseVolatilityPortfolioStrategy | Final Value / Orders | EXACT |
