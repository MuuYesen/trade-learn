...
如果你已经有 Backtrader 策略，可以先从 `tradelearn.engine` 迁移；如果你只是想快速验证一个因子或组合权重逻辑，可以从 `tradelearn.lite` 开始；如果你正在做机器学习策略，`tradelearn.research + tradelearn.ml + MLflow + report` 可以把训练、因果筛选、评分、调仓、回测和结果追踪放在同一条线上。
