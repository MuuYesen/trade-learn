...
def get_bt_strategy(BaseClass):
    class EMA_Cross_Strategy(BaseClass):
        ...
        def next(self):
            ...
            crosses_up = (
                not self.position
                and self.ema9[-2] <= self.ema21[-2]
                and self.ema9[-1] > self.ema21[-1]
            )
            crosses_down = (
                self.position and self.ema9[-2] >= self.ema21[-2] and self.ema9[-1] < self.ema21[-1]
            )
            if crosses_up:
                self.buy()
            elif crosses_down:
...
class EMA_Cross_Core(CoreStrategy):
    ...
    def next(self):
        ...
        crosses_up = (
            not self.getposition().size
            and self.ema9[cursor - 1] <= self.ema21[cursor - 1]
            and self.ema9[cursor] > self.ema21[cursor]
        )
        crosses_down = (
