...
| `tradelearn.factor` | Alpha 公式与因子分析工具。 | `FactorAnalyzer`, `MultiFactorAnalyzer`, `MultiPeriodFactorAnalyzer`, `FactorRiskModel`, `PerformanceAttribution`, `alpha101`, `alpha191`, `clean_factor_and_forward_returns` | [Factor](reference/factor.md) |
| `tradelearn.report` | HTML、Excel 与研究报告导出。 | `Reporter`, `ReportContext`, `ReportSection` | [Report](reference/report.md) |
| `tradelearn.research` | 指数增强研发流程记录、预处理与组合构建工具。 | `Pipeline`, `FeatureBuilder`, `FeatureSet`, `ModelScorer`, `ResearchResult`, `ResearchRun`, `ResearchStep`, `Transformer`, `current_run`, `derive`, ... (+7) | [Research](reference/research.md) |
| `tradelearn.ml` | 机器学习、模型注册与特征筛选。 | `CausalSelector`, `AutoML`, `ModelLoader`, `ModelRegistry`, `model_uri` | [ML](reference/ml.md) |
