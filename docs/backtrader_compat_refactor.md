...
- **解决指标访问黑洞**：
    - **预计算与热路径分离**：指标（SMA/EMA等）依然保持向量化预计算的优势，但在 `next()` 循环的“热路径”中，彻底废弃了基于属性（Attribute）的动态定位。
    - **消除动态查找开销**：重构了 `IndicatorLine`，消除了原本每一根 K 线都要触发的 `@property`、`getattr` 和 `hasattr` 调用。现在指标线直接通过 NumPy 指针偏移量（Offset）访问预计算结果。
- **延迟日期转换**：消灭了 `pd.to_datetime` 在循环中的冗余调用，仅在用户显式调用 `.date()` 时进行转换。
