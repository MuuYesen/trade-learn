...
本页由 `scripts/generate_api_reference.py` 从 `tradelearn.lite` 运行时代码签名生成。
...
| `stats_mode` | `str` | `'full'` | `full` 返回完整 orders/fills/trades/equity; `lazy` 只计算轻量 summary。 |
...
返回值：`LiteStats`,支持 `stats['final_value']` 等 summary key,并提供 `summary`、`equity`、`returns`、`fills`、`trades`、`positions`、`orders`、`records`、`strategy`、`config`。
...
返回值：`IndicatorProxy` 或 `IndicatorBundle`。
...
返回值：`Order` 或 bracket `list[Order]`。
...
| 成员 | 类型 | 签名 | 说明 |
|---|---|---|---|
| `Backtest.run` | 方法 | `(self, **kwargs) -> 'LiteStats'` | Run the backtest and return statistics. |
| `Backtest.optimize` | 方法 | `(self, **kwargs) -> 'LiteStats'` | Simple grid search optimization. |
| `Backtest.plot` | 方法 | `(self, *args, **kwargs)` | Return market replay charts for the most recent Lite run. |
| `Backtest.report` | 方法 | `(self, path: 'str' = 'report.html', benchmark=None, sections=None)` | Write a Tradelearn report for the most recent Lite run. |

### Strategy

| 成员 | 类型 | 签名 | 说明 |
|---|---|---|---|
| `Strategy.position` | 方法 | `(self, ticker: 'str | None' = None) -> 'Any'` | Return the Tradelearn Lite position view for a ticker. |
| `Strategy.I` | 方法 | `(self, funcval: 'Callable | pd.DataFrame | pd.Series | Any', *args, name: 'str | None' = None, plot: 'bool' = True, overlay: 'bool | None' = None, color: 'str | None' = None, scatter: 'bool' = False, **kwargs) -> 'Any'` | Declare a Tradelearn Lite gradually revealed indicator. |
| `Strategy.buy` | 方法 | `(self, *, ticker: 'str' = None, size: 'float' = 1.0, limit: 'float' = None, stop: 'float' = None, sl: 'float' = None, tp: 'float' = None, tag: 'object' = None)` |  |
| `Strategy.sell` | 方法 | `(self, *, ticker: 'str' = None, size: 'float' = 1.0, limit: 'float' = None, stop: 'float' = None, sl: 'float' = None, tp: 'float' = None, tag: 'object' = None)` |  |
| `Strategy.cancel` | 方法 | `(self, order: 'Any') -> 'None'` |  |
| `Strategy.order_target_size` | 方法 | `(self, *, ticker: 'str' = None, target: 'float' = 0, **kwargs: 'Any')` |  |
| `Strategy.order_target_value` | 方法 | `(self, *, ticker: 'str' = None, target: 'float' = 0.0, price: 'float | None' = None, **kwargs: 'Any')` |  |
| `Strategy.order_target_percent` | 方法 | `(self, *, ticker: 'str' = None, target: 'float' = 0.0, **kwargs: 'Any')` |  |
| `Strategy.target_percent` | 方法 | `(self, ticker: 'str', target: 'float')` | Move one ticker toward a target portfolio weight. |
| `Strategy.target_weights` | 方法 | `(self, weights: 'Mapping[str, float] | pd.Series', *, close_missing: 'bool' = True) -> 'list[Any]'` | Move the portfolio toward the requested ticker weights. |
| `Strategy.target_equal` | 方法 | `(self, tickers: 'Sequence[str]', *, weight: 'float' = 1.0, close_missing: 'bool' = True) -> 'list[Any]'` | Assign an equal combined target weight to the selected tickers. |
| `Strategy.close_all` | 方法 | `(self) -> 'list[Any]'` | Close all known Lite data-feed positions. |
| `Strategy.buy_bracket` | 方法 | `(self, *, ticker: 'str' = None, size: 'float' = 1.0, limit: 'float' = None, stop: 'float' = None, sl: 'float' = None, tp: 'float' = None, tag: 'object' = None) -> 'list[Any]'` |  |
| `Strategy.sell_bracket` | 方法 | `(self, *, ticker: 'str' = None, size: 'float' = 1.0, limit: 'float' = None, stop: 'float' = None, sl: 'float' = None, tp: 'float' = None, tag: 'object' = None) -> 'list[Any]'` |  |
| `Strategy.record` | 方法 | `(self, name: 'str' = None, plot: 'bool' = True, overlay: 'bool' = None, color: 'str' = None, scatter: 'bool' = False, **kwargs) -> 'None'` |  |
| `Strategy.equity` | 属性 | `` |  |
| `Strategy.storage` | 属性 | `` |  |
| `Strategy.orders` | 属性 | `` |  |
| `Strategy.trades` | 方法 | `(self, ticker: 'str' = None) -> 'tuple[Any, ...]'` |  |
| `Strategy.closed_trades` | 属性 | `` |  |
| `Strategy.start_on_day` | 方法 | `(self, n: 'int') -> 'None'` |  |
| `Strategy.start_on_bar` | 方法 | `(self, n: 'int') -> 'None'` | Start ``next`` no earlier than bar index ``n``. |
| `Strategy.prepare_data` | 方法 | `(tickers: 'list[str]', start: 'str') -> 'pd.DataFrame | None'` |  |
...
|---|---|---|---|
| `LiteDataProxy.df` | 属性 | `` |  |
| `LiteDataProxy.index` | 属性 | `` |  |
| `LiteDataProxy.now` | 属性 | `` |  |
| `LiteDataProxy.tickers` | 属性 | `` |  |
| `LiteDataProxy.the_ticker` | 属性 | `` |  |

### PositionProxy

| 成员 | 类型 | 签名 | 说明 |
|---|---|---|---|
| `PositionProxy.size` | 属性 | `` |  |
| `PositionProxy.close` | 方法 | `(self, portion: 'float' = 1.0)` |  |
| `PositionProxy.pl` | 属性 | `` |  |
| `PositionProxy.pl_pct` | 属性 | `` |  |
| `PositionProxy.is_long` | 属性 | `` |  |
| `PositionProxy.is_short` | 属性 | `` |  |
