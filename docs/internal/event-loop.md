...
## 1.1 实盘兼容 EventRunner 主线

### 目标

将回测与实盘统一到同一个单事件 `EventRunner` 核心，同时保持用户 API 100% 不变。
`strategy.next()`、`self.data[0]`、`self.buy()` 等方法签名与语义均冻结，策略作者无需
改动任何代码。

`EventRunner` 的基本单位是单根 bar / 单个 broker event，而不是历史批次。历史回测
可以在 driver 层使用批量读取或预计算，但核心事件语义必须与实盘一致。

### 动机

当前 1.0 架构中 Python for 循环每根 bar 承担调度开销，Rust 撮合核主要覆盖撮合与
记账。Stage 10 优化不能牺牲 trade-learn 的事件驱动定位，也不能把回测和实盘拆成两套
策略执行模型。

因此主线目标调整为:

- 回测与实盘共用同一个 `EventRunner.on_bar()` / `on_broker_event()` 顺序。
- Rust 负责单事件推进、订单队列、撮合、mark-to-market 与 broker event 标准化。
- Python 策略逻辑仍保留在 `strategy.next()`，不引入 DSL 或策略子集下沉。
- 回测专用批量优化只允许作为 driver 层加速，不得改变核心事件语义。
...
HistoricalDriver / LiveDriver
  └─ EventRunner.on_bar(bar)
       ├─ advance rolling/shared bar buffer
       ├─ poll/drain BrokerEventPump
       ├─ match pending orders 或接收实盘成交回报
       ├─ mark_to_market
       ├─ advance indicators
       ├─ PyO3 → strategy._pre_next()
       ├─ PyO3 → strategy.next()
       └─ enqueue strategy orders
```
...
BacktestBatchDriver
  └─ 预读取/预计算历史数据
  └─ 按单事件顺序调用 EventRunner.on_bar(...)
```

该 batch driver 只能减少数据读取、cursor plan、指标预计算等外围开销，不得跳过
`strategy.next()`，也不得让 batch 内订单提前影响前序 bar。

### 兼容性保证

- `strategy.next()`、`self.data[0]/[-1]`、`self.buy()/sell()/close()` 签名和语义不变。
- `self.fast[0]` 行为不变，通过统一 line cursor 提前推进。
- 暖机期语义不变：最长 lookback 前只推进数据缓冲，不调用户 `next()`。
- 多 data feed primary clock 对齐语义不变。实盘异步数据源必须显式选择
  latest-at-or-before、等待齐套或多次触发三种策略之一，默认沿用回测
  latest-at-or-before。
- 回测 `BatchIndicatorCache` 与实盘 `RollingIndicatorCache` 暴露同一种 line 读取接口。
- `compat.backtrader` 迁移策略零改动可运行。

### 关键实现约束

1. **EventRunner**：统一单事件核心，拥有 `on_bar()`、`on_broker_event()`、
   `submit_order()` 与 `snapshot()` 边界。
2. **Driver 分层**：`HistoricalDriver`、`LiveDriver`、`PaperDriver` 只负责事件来源，
   不拥有策略语义。
3. **共享 rolling bar buffer**：回测可预装数组，实盘以 append/overwrite 环形方式更新；
   Python line 读取接口一致。
4. **指标缓存双模式**：
   - `BatchIndicatorCache`：回测全量向量预计算，适配 pandas-ta-classic / TDX / TV。
   - `RollingIndicatorCache`：实盘 rolling window 重算最近窗口，避免未来函数。
5. **BrokerEventPump 标准化**：fill/cancel/reject/order status 全部通过统一事件泵进入
   runner；QMT 等实盘 broker 只实现 poller，不污染核心事件循环。
6. **GIL 策略**：bar 内持有 GIL 完成 Python callback；不得在持有 GIL 时执行阻塞 I/O。
7. **订单队列**：`next()` 中 `self.buy()` 产生的订单在事件结束后入队；普通订单从下一
   bar 起具备撮合资格，`trade_on_close` 保持现有特殊规则。

### 验收标准

- golden 50/50 compare 仍通过（trades 0 差异，PnL rtol=1e-4）。
- 现有全部单元测试与 golden 测试通过，无策略代码改动。
- benchmark（sma_cross + AAPL 10000 bars，20 次中位数）默认事件驱动路径:
  - vs 当前 Python callback loop ≥ 1.5x
  - vs Backtrader oracle ≥ 4x
  - 指标密集策略在 BatchIndicatorCache 接入后目标 ≥ 5x
- `compat.backtrader` 十个迁移金标策略零改动运行通过。
- live/paper dry-run 能以模拟 broker event pump 跑通同一策略生命周期。
