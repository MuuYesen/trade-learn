...
class MACD_Volume_Strategy(Strategy):
    ...
    def next(self):
        ...
        atr = self.atr[-1]
        macd_line = self.macd_line_ind[-1]
        macd_signal = self.macd_signal_ind[-1]
        macd_line_prev = self.macd_line_ind[-2] if len(self.macd_line_ind) > 1 else macd_line
        macd_signal_prev = self.macd_signal_ind[-2] if len(self.macd_signal_ind) > 1 else macd_signal
