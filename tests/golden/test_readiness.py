...
def test_golden_readiness_script_runs_without_pythonpath() -> None:
    env = os.environ.copy()
...
def test_golden_readiness_reports_missing_real_artifacts_as_blocked(tmp_path: Path) -> None:
    ...
    assert payload["summary"]["strategies_ready"] == 10
...
def test_golden_readiness_text_output_is_actionable(tmp_path: Path) -> None:
    ...
    assert "strategies=10/10" in result.stdout
...
def test_golden_readiness_can_target_tv_expected_subset(tmp_path: Path) -> None:
    datasets_root = tmp_path / "datasets"
    expected_root = tmp_path / "expected"
    tv_dir = datasets_root / "tv"
    tv_dir.mkdir(parents=True)
    ...
    for dataset in manifest["datasets"]:
        if dataset["engine"] != "tv":
            continue
        tv_symbols.append(dataset["symbol"])
        filename = (
            f"{dataset['symbol']}_{dataset['start']}_{dataset['end']}_{dataset['freq']}.parquet"
        )
        frame.to_parquet(tv_dir / filename)
    for strategy in manifest["strategies"]:
        for symbol in tv_symbols:
            (expected_root / f"{strategy['name']}__{symbol}.json").write_text("{}")
    ...
    assert payload["summary"]["datasets_total"] == 5
    assert payload["summary"]["datasets_ready"] == 5
    assert payload["summary"]["expected_total"] == 50
    assert payload["summary"]["expected_ready"] == 50
    assert payload["summary"]["strategies_ready"] == 10
