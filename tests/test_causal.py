...
class TestCuasal(unittest.TestCase):
    ...
    def test_causal_blanket_iamb(self):
        ...
        data = Blanket.fit_causal(data, method='iamb', is_discrete=False)
        print(data)

    def test_causal_blanket_pcmb(self):
        data = Query.read_csv('./data/600036SH.csv', begin='2020-01-01', end='2023-06-21')
        data = data.drop(columns=['code'])
        data = Blanket.fit_causal(data, method='pcmb', is_discrete=False)
        print(data)

    def test_causal_graph_pc(self):
        data = Query.read_csv('./data/600036SH.csv', begin='2020-01-01', end='2023-06-21')
        data = data.drop(columns=['code'])
        Graph.fit_causal(data, method='pc', is_discrete=False)

    def test_causal_graph_ges(self):
        data = Query.read_csv('./data/600036SH.csv', begin='2020-01-01', end='2023-06-21')
        data = data.drop(columns=['code'])
        Graph.fit_causal(data, method='ges', is_discrete=False)
