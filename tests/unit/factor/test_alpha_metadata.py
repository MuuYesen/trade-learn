...
def test_render_alpha_known_differences_script_check_ignores_fenced_headings(
    tmp_path: Path,
) -> None:
    """The known differences check ignores Alpha headings inside code fences."""
    rendered = subprocess.run(
        [
            sys.executable,
            "scripts/render_alpha_known_differences.py",
...
def test_render_alpha_known_differences_script_check_ignores_nested_fenced_headings(
    tmp_path: Path,
) -> None:
    """The known differences check keeps shorter fences inside longer fences."""
    rendered = subprocess.run(
        [
            sys.executable,
            "scripts/render_alpha_known_differences.py",
