...
def build_lab_plan(
    ...
    *,
    ...
    mlflow_env = {"MLFLOW_TRACKING_URI": config.mlflow_tracking_uri}
    jupyter_args = [
        "python",
        "-m",
        "jupyterlab",
    ...
    return LabPlan(
        jupyter=LabCommand(tuple(jupyter_args), mlflow_env),
        mcp=LabCommand(("tradelearn", "mcp", "--dry-run"), mlflow_env),
