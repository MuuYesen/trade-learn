...
class MiPlot:
    ...
    @staticmethod
    def _nullity_sort(df, sort=None, axis='columns'):
        """
    ...
    @staticmethod
    def _nullity_filter(df, filter=None, p=0, n=0):
        """
    ...
    def matrix(
        ...
        """
        df = MiPlot._nullity_filter(df, filter=filter, n=n, p=p)
        df = MiPlot._nullity_sort(df, sort=sort, axis='columns')
    ...
    def bar(
        ...
        """
        df = MiPlot._nullity_filter(df, filter=filter, n=n, p=p)
        df = MiPlot._nullity_sort(df, sort=sort, axis='rows')
