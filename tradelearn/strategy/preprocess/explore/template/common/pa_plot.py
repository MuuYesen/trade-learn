...
class PaPlot:
    ...
    @staticmethod
    def _missing_vals(data: pd.DataFrame) -> Dict:
    ...
    def corr_mat(
        ...
        corr = PaPlot._corr_selector(corr, split=split, threshold=threshold)
    ...
    def missingval_plot(  # noqa: PLR0915
        ...
        mv_total, mv_rows, mv_cols, _, mv_cols_ratio = PaPlot._missing_vals(data).values()
...
df = pd.read_csv('/Users/muyesen/MAIN/Project/Personal/trade-learn/tests/data/600036SH.csv')
# cat_plot(df)
# corr_mat(df) # returns a color-encoded correlation matrix
corr_plot(df) # returns a color-encoded heatmap, ideal for correlations
# corr_interactive_plot(df, split="neg").show() # returns an interactive correlation plot using plotly
# dist_plot(df) # returns a distribution plot for every numeric feature
# missingval_plot(df)
plt.show()
