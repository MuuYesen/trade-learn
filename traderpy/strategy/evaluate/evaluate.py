    ...
    def factor_compare(data):
        ...
        for col in data.columns:
            if not col.startswith('alpha'):
                continue
            try:
                factor_data = al.utils.get_clean_factor_and_forward_returns(data[col], close, quantiles=5, periods=(5,), max_loss=1)
...
if __name__ == '__main__':
    s_data = pd.read_csv('../../database/data/000300SH.csv', parse_dates=['date'], dtype={'code': str}, low_memory=True, encoding='utf_8_sig')
    s_data.rename(columns={'open': 'alpha_open'}, inplace=True)
    res = Evaluate.factor_compare(s_data)
    print(res)
