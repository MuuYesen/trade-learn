...
from .tabs import AnalyzerTab, MetadataTab, LogTab, SourceTab, ConfigTab
...
class BacktraderPlotting(metaclass=bt.MetaParams):
    ...
    def __init__(self, **kwargs):
        ...
        if self.p.use_default_tabs:
            self.tabs = [
                AnalyzerTab, MetadataTab, LogTab, ConfigTab]
        if self.p.show_source:
            self.tabs += [SourceTab]
    ...
    def _output_plotfile(self, model, figid=0, filename=None,
        ...
        '''
        if filename is None:
            filename = f'bt_bokeh_plot_{figid}.html'
